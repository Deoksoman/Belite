...
class Minimap():
    def __init__(self,model_path):
        self.minimap = cv2.imread('/Users/chan/tennis/football_keypoint_pkl/minimap/MINIMAP.png')
        self.model_k = YOLO(model_path)
        #여기서 어느정도의 변화가 있을 때 호모그래피를 업데이트 할 것인지 결정합니다.
        self.keypoints_displacement_mean_tol = 5
        #키포인트 제이슨
        with open('/Users/chan/tennis/football_keypoint_pkl/minimap/keypoint.json', 'r') as f:
            self.keypoints_map_pos = json.load(f)
        #키포인트 야믈파일
        with open('/Users/chan/tennis/football_keypoint_pkl/minimap/keyloint.yaml', 'r') as file:
